...
#define MMC_CD			!(PINB & (1 << 5))	/* Test if card detected.   yes:true, no:false, default:true */
#define MMC_WP			0					/* Test if write protected. yes:true, no:false, default:false */
#define	FCLK_SLOW()		SPCR = (0 << SPIE) | (1 << SPE) | (1 << MSTR) | (1 << SPR1) | (1 << SPR0); SPSR &= ~(1 << SPI2X);	/* Set SPI slow clock (100-400kHz) */
...
void power_on (void)
{
	if (debugFlag_fatfs) debugString_P(PSTR("FAT FS: power_on(): Initialising SPI and powering up card\r\n"));
	
	/* Configure MOSI/MISO/SCLK/CS pins */
	DDRB |= (1 << PB0);		// Output - CS0
	DDRB |= (1 << PB1);		// Output - CLK
	DDRB |= (1 << PB2);		// Output - MOSI
	DDRB &= ~(1 << PB3);	// Input - MISO
	DDRB |= (1 << PB4);		// Output - Power
	DDRB &= ~(1 << PB5);	// Input - CD
	
	// Enable pull ups on MISO and CD
	PORTB |= (1 << PB3);
	PORTB |= (1 << PB5);
	
	/* Turn socket power on */
	PORTB &= ~_BV(4);
...
void power_off (void)
{
	if (debugFlag_fatfs) debugString_P(PSTR("FAT FS: power_off(): Powering down SD card\r\n"));
	
	/* Disable SPI function */
	SPCR = 0;

	/* De-configure MOSI/MISO/SCLK/CS pins (set hi-z) */
	DDRB &= ~(1 << PB0);		// Input - CS0
	DDRB &= ~(1 << PB1);		// Input - CLK
	DDRB &= ~(1 << PB2);		// Input - MOSI
	DDRB &= ~(1 << PB3);		// Input - MISO
	
	// Disable pull ups on MISO and CD
	PORTB &= ~(1 << PB3);
	PORTB &= ~(1 << PB5);
...
DSTATUS mmc_disk_initialize (void)
{
	...
	if (debugFlag_fatfs) debugString_P(PSTR("FAT FS: mmc_disk_initialize(): Executing\r\n"));
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Put the card SPI mode */
		...
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* Is the card SDv2? */
			...
			if (debugFlag_fatfs) debugString_P(PSTR("FAT FS: mmc_disk_initialize(): SD Card did not respond to CMD8\r\n"));
			if (send_cmd(ACMD41, 0) <= 1) 	{
				ty = CT_SD1; cmd = ACMD41;	/* SDv1 */
				if (debugFlag_fatfs) debugString_P(PSTR("FAT FS: mmc_disk_initialize(): SD Card reports SDv1 type\r\n"));
			} else {
	...
	if (ty) {			/* Initialization succeeded */
		...
		FCLK_FAST();
		if (debugFlag_fatfs) debugString_P(PSTR("FAT FS: mmc_disk_initialize(): Successful\r\n"));
	} else {			/* Initialization failed */
		power_off();
		if (debugFlag_fatfs) debugString_P(PSTR("FAT FS: mmc_disk_initialize(): Failed\r\n"));
	}
